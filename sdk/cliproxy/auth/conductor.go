...
func (m *Manager) executeStreamWithModelPool(ctx context.Context, executor ProviderExecutor, auth *Auth, provider string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, routeModel string, execModels []string, pooled bool) (*cliproxyexecutor.StreamResult, error) {
	...
	for idx, execModel := range execModels {
		...
		for bootstrapAttempt := 0; ; bootstrapAttempt++ {
			execReqAttempt := execReq
			execOpts := opts
			execReqAttempt, execOpts = applyRequestAfterAuthInterceptor(ctx, executor, provider, execReqAttempt, execOpts, requestedModelAliasFromOptions(execOpts, routeModel))
			streamResult, errStream := executor.ExecuteStream(ctx, auth, execReqAttempt, execOpts)
			if errStream != nil {
...
func (m *Manager) executeMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int) (cliproxyexecutor.Response, error) {
	...
	var lastErr error
	for {
		if !homeMode && maxRetryCredentials > 0 && len(attempted) >= maxRetryCredentials && !bypassCredentialAttemptLimit(lastErr) {
			if lastErr != nil {
				return cliproxyexecutor.Response{}, lastErr
			}
			return cliproxyexecutor.Response{}, &Error{Code: "auth_not_found", Message: "no auth available"}
